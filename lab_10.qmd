...
#Join the shot_spotter and school days and filter it

shootings_by_school_days <- shot_spotter %>%
  inner_join(dc_school_days_2019, by="date") %>% 
  filter(between(hour, 7, 16)) %>% 
  filter(school_status == "school_open")

dc_schools_buffered <- washington_schools %>%
  mutate(geometry = st_buffer(geometry, dist=200))

schools_and_shootings <- dc_schools_buffered %>%
  st_join(shot_spotter, left="FALSE") %>% 
  group_by(school_name)

schools_and_shootings
```
...
#There were 893 distinct shooting events within 200 meters of any DC school in 2019 during school hours. 
...
#There were 147 distinct schools that had shooting events within 200 meters of any DC school in 2019 during school hours. 

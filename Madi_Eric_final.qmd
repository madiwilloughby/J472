...
leaflet() %>%
  ...
  addPolygons(data=county_wealth_2020, 
              weight = 2, 
              fillColor = ~pal(estimate_2020), 
              fillOpacity = 1, 
              label = paste("Median household income in 2020 was :",(scales::dollar(county_wealth_2020$estimate_2020)), "for",county_wealth_2020$name)) %>%
  addLegend(pal = pal, values = county_wealth_2020$estimate_2020,  position = "bottomleft", title = "2020 Median Household Income", bins=5)

#This map shows that wealth is pretty concentrated in the middle of the state, which makes sense since those are the more urban areas. Howard, Calvert, and Montgomery counties ranked the highest in median household income in 2020. Somerset and Allegany counties had the lowest median income, followed by Baltimore City (which is interesting given that it is more urban than other areas in the state)
...
#These two summary tables showed that the three counties that were not predominantly white (Charles, Prince George's, and Baltimore City)  had larger wealth increases than predominantly white counties from 2010 to 2020. The average increase was almost four percentage points higher (23.75% vs. 19.84%)

no_predom_white_geography <- maryland_county_boundaries %>% 
  inner_join(wealth_change_by_race, by = "geoid") %>% 
  filter(predom_white == "NO") %>% 
  clean_names()
  
predom_white_geography <- maryland_county_boundaries %>% 
  inner_join(wealth_change_by_race, by = "geoid") %>% 
  filter(predom_white == "YES") %>% 
  clean_names()
...
ggplot() +
  ...
  geom_sf(data = no_predom_white_geography, aes(fill=change_2010_2020)) +
    labs(title = "Median Household Income Changes in Non-Predominantly White Counties")


ggplot() +
  geom_sf(data=maryland_county_boundaries) +
  geom_sf(data = predom_white_geography, aes(fill=change_2010_2020)) +
  labs(title = "Median Household Income Changes in Predominantly White Counties")

#These two maps show income changes in predominantly white counties and counties that aren't predominantly white. It shows that while the average income change was higher in the three counties that weren't predominantly white, several predominantly white counties outpaced Prince George's and Charles counties. This connects to the data that shows predominantly white counties make up the second through sixth highest changes in median income. So, while predominantly white counties on average fared worse than other counties, the specifics show many predominantly white counties did better. This could mean that average is not the best measure of income changes in this context, especially because there are only three counties that aren't predominantly white and 21 counties that are predominantly white.
...
#This shows more populated counties saw greater increases in median income than counties with a below average population. The average increase among counties with an above average population was 23.31%, compared to 18.13% for counties with a below average population.
```
...
#Load the data by county.

house_price_index_md_counties <- rio::import("https://docs.google.com/spreadsheets/d/1UPd5M5LlXUvBcxYXBpHsp30L9P_h1r3A6rMG8QOvoh0/edit?usp=sharing", which = "house_price_index_md")

house_price_index_md_counties$geoid <- as.numeric(house_price_index_md_counties$geoid)

#The largest increase in house price in Prince George's County with a 40.13%, followed by Frederick County with a 20.63% increase and then Montgomery County with a 18.70% increase.
...
#Summarize the #SPELLING# percntage changes for each county.
...
#With the median percentage change of median household incomes in each county being about 20% and the first quartile being about 18.7%, the median household incomes mostly exceeded the increase in the housing price index of 15.4%.

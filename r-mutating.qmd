...
city_md_income <- read.csv("city_md_income.csv")
...
#To find the highest absolute change
city_md_income %>%
  select(place, median_inc_2010, median_inc_2020) %>%
    mutate(Diff_Income = (median_inc_2020-median_inc_2010)) %>% 
    mutate(Pct_Change = ((median_inc_2020-median_inc_2010)/(median_inc_2010)*100)) %>% 
...
#To find the highest percentage change
city_md_income %>%
  select(place, median_inc_2010, median_inc_2020) %>%
    mutate(Diff_Income = (median_inc_2020-median_inc_2010)) %>% 
    mutate(Pct_Change = ((median_inc_2020-median_inc_2010)/(median_inc_2010)*100)) %>% 
...
#The city with the highest absolute change is Beaver Creek CDP with an absolute change of 118,292. The city with the highest percentage change is Shaft CDP with about a 400% increase.
...
median(city_md_income$Diff_Income,na.rm=TRUE)
median(city_md_income$Pct_Change,na.rm=TRUE)

#To find the median values for the percentage change and absolute differences, you have to define the columns by overwriting them so the columns from the mutate feature are placed into the data set permanently. After this is defined, the median can then be found using the median code.
